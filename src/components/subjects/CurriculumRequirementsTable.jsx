...
function getIntegerInputValue(id) {
  return Number.parseInt(String(document.getElementById(id)?.value ?? ""), 10);
}

export default function CurriculumRequirementsTable({ requirements = [] }) {
  ...
  const isProcessing =
    Boolean(editingRequirementId) ||
    Boolean(deletingRequirementId) ||
    isRefreshing;
  ...
  async function handleEdit(requirement) {
    ...
    try {
      const result = await Swal.fire({
            ...
            <div class="text-left">
              <div class="mb-5 rounded-xl bg-slate-100 p-4">
                ...
                <p class="mt-1 font-semibold text-slate-900">
                  ${escapeHtml(requirement.subject?.name || "Sin materia")}
                  ·
                  ${escapeHtml(requirement.grade_level?.name || "Sin grado")}
                </p>
              </div>
        ...
        didOpen: () => {
          const weeklyInput = document.getElementById("swal-weekly-periods");

          const maxDailyInput = document.getElementById("swal-max-daily");

          const minDaysInput = document.getElementById("swal-min-days");

          const blockSizeInput = document.getElementById("swal-block-size");

          const consecutiveInput = document.getElementById(
            "swal-allow-consecutive",
          );

          if (weeklyInput) {
            weeklyInput.value = String(requirement.weekly_periods ?? 1);
          }

          if (maxDailyInput) {
            maxDailyInput.value = String(requirement.max_periods_per_day ?? 1);
          }

          if (minDaysInput) {
            minDaysInput.value = String(requirement.min_days_per_week ?? 1);
          }

          if (blockSizeInput) {
            blockSizeInput.value = String(
              requirement.preferred_block_size ?? 1,
            );
          }

          if (consecutiveInput) {
            consecutiveInput.checked = Boolean(
        ...
        preConfirm: () => {
          const weeklyPeriods = getIntegerInputValue("swal-weekly-periods");

          const maxPeriodsPerDay = getIntegerInputValue("swal-max-daily");

          const minDaysPerWeek = getIntegerInputValue("swal-min-days");

          const preferredBlockSize = getIntegerInputValue("swal-block-size");

          const allowConsecutivePeriods = Boolean(
            document.getElementById("swal-allow-consecutive")?.checked,
          );

          if (!Number.isInteger(weeklyPeriods) || weeklyPeriods < 1) {
          ...
          if (!Number.isInteger(maxPeriodsPerDay) || maxPeriodsPerDay < 1) {
            Swal.showValidationMessage(
          ...
          if (!Number.isInteger(preferredBlockSize) || preferredBlockSize < 1) {
            Swal.showValidationMessage(
          ...
          if (!allowConsecutivePeriods && preferredBlockSize > 1) {
            Swal.showValidationMessage(
              "Activa las horas consecutivas para utilizar bloques mayores a uno.",
            );
      ...
      Swal.fire({
        ...
        text: "Espera mientras se actualiza la carga curricular.",
      ...
      formData.set("weeklyPeriods", String(result.value.weeklyPeriods));

      formData.set("maxPeriodsPerDay", String(result.value.maxPeriodsPerDay));

      formData.set("minDaysPerWeek", String(result.value.minDaysPerWeek));

      formData.set(
        "preferredBlockSize",
        String(result.value.preferredBlockSize),
      );

      formData.set(
        "allowConsecutivePeriods",
        String(result.value.allowConsecutivePeriods),
      );

      const updateResult = await updateCurriculumRequirementAction(formData);
      ...
      await showSuccessAlert({
        title: "Carga curricular actualizada",
      ...
      await showErrorAlert({
        title: "No fue posible actualizar",
        text: "Ocurrió un error inesperado al actualizar la carga.",
      });
    } finally {
  ...
  async function handleDelete(requirement) {
    ...
    const confirmation = await Swal.fire({
      ...
      html: `
          <p>
            Se eliminará la carga de
            <strong>
              ${escapeHtml(requirement.subject?.name || "la materia")}
            </strong>
            para
            <strong>
              ${escapeHtml(requirement.grade_level?.name || "el grado")}
            </strong>.
          </p>

          <p style="margin-top:12px; color:#b45309; font-size:13px;">
            También se eliminarán las asignaciones y clases relacionadas con esta carga.
          </p>
        `,
    ...
    try {
      ...
      const deleteResult = await deleteCurriculumRequirementAction(formData);
      ...
      await showErrorAlert({
        title: "No fue posible eliminar",
        text: "Ocurrió un error inesperado al eliminar la carga.",
      });
    } finally {
  ...
  if (!requirements.length) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center">
        <CalendarRange className="mx-auto text-slate-400" size={34} />
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 px-5 py-5 sm:px-6">
        <h3 className="font-bold text-slate-900">Carga curricular</h3>
        ...
        <table className="w-full min-w-[1050px] text-left">
          <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-6 py-4">Semanales</th>

              <th className="px-6 py-4">Máximo diario</th>

              <th className="px-6 py-4">Mínimo días</th>

              <th className="px-6 py-4">Consecutivas</th>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <span
                        ...
                        style={{
                          backgroundColor:
                            requirement.subject?.color || "#334155",
                        }}
                      />

                      <p className="font-semibold text-slate-900">
                        {requirement.subject?.name || "Sin materia"}
                      </p>
                    </div>
                  </td>

                  <td className="px-6 py-4 text-sm text-slate-600">
                    {requirement.grade_level?.name || "Sin grado"}
                  </td>
